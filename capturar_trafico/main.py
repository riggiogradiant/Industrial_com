...
# # Variable global para el control de captura
...
#     scapy.sniff(iface=interface, filter=filter_expr, store=False, prn=sniffed_packet, timeout=duration)
    
#     # Desactiva la captura después del tiempo especificado
...
#     print(f"Capturando tráfico en la interfaz {loopback_interface} para el puerto 502 durante {capture_duration} segundos...")
    
#     # Ejecuta la captura en un hilo separado
#     sniff_thread = threading.Thread(target=sniff_packets, args=(loopback_interface, capture_duration))
#     sniff_thread.start()
